...
export class HomeStore {
    ...
    updateTask(task: TaskInfo): Promise<TaskInfo> {
        return new Promise((resolve, reject) => {
            ...
            let step = TaskMapById[taskID - 1].step; // ？ step
            let hasExceed = updateTask.isDone;
            Logger.error("HomeViewModel.ets", JSON.stringify(step));
            if (step === 0) {
                ...
                Logger.error("HomeViewModel.ets finValue:", JSON.stringify(finValue), "and value", JSON.stringify(value));
                updateTask.isDone = updateTask.isDone || value >= Number(targetValue); // ?
                updateTask.finValue = (updateTask.isDone ? targetValue : `${value}`).substring(0, 4);
            }
